...
if (isset($_POST['username'])){
 ...
 //Checking is user existing in the database or not
        $query = "SELECT restname FROM `restregister` WHERE ownname='". $username ."' and password='". md5($password) ."'";
$result = mysqli_query($con,$query) or die(mysqli_error($con));
 $rows = mysqli_num_rows($result);
$row = mysqli_fetch_assoc($result);
$_SESSION['restaname'] = $row['restname'];
 
        if($rows==1){
     $_SESSION['username'] = $username;

            // Redirect user to index.php
      header("Location: indexauth1.php");
         }else{
 echo "<div class='form'>
...
<a href="menu.php">Menu</a>
  ...
  <div class="dropdown">
    ...
    <div class="dropdown-content">
      <a   href="Customer Reg.php">Customer</a>
...
<h3 style="text-align:center"><u>Restaurant Login</u></h3>
<input type="text" placeholder="username" name="username"/>
<input type="password" placeholder="password" name="password"/>
<button>login</button>
<p class="message">Not registered? <a href="Restaurant Reg.php">Create an account</a></p>
</form>
</div>
</div>
<footer>
